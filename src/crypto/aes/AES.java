...
>>>>>>> aes
	...
	public void keyExpansion() throws Exception {
...
=======
		...
		int len = (Nb); // Nb = 32bit => 4 * 8bit = 4 * 1byte
		len *= Nr + 1;
		for (int i = this.key.length; i < len; i++) {
...
>>>>>>> aes
	...
	public void encrypt() {
		//addRoundKey(state, rKey, inverse);
	...
	private byte[][] subBytes(byte[][] state, boolean inverse) throws Exception
	{
		...
		for (int i = 0; i < 4; i++ ){
			for (int j = 0; j < 4; j++){ //i = Zeile, j = Spalte
				...
				int col = val & 0x0f;
				if(inverse){
					out[i][j] = (byte) isbox[row][col];
				} else {
					out[i][j] = (byte) sbox[row][col];
				}
			}
		}
		return out;
